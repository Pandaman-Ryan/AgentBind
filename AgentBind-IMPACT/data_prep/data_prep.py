...
class datasetPrep(converter_template):
    ...
    def prepare_labelled_samples(self, dir_data, TF_ID):
        ...
        labelled_list_pos, n_pos = self._readFromImpact(data_path, 1)

        data_path = "%s/IMPACT/train_test_negative_bed_%sonly_center.txt" %(dir_data, TF_ID)
        labelled_list_neg, n_neg = self._readFromImpact(data_path, 0)
        labelled_list = labelled_list_pos + labelled_list_neg
    ...
    def convert(self, samples_coord, len_seq, dir_data):
        ...
        for smp in samples_coord:
            ...
            if (chromID in chrom_length_dict) \
                ...
                sequences.append((chromID, seq, label))
    ...
    def make_datasets(self, samples):
        ...
        for smpl in samples:
            (chromID, seq, label) = smpl
            dice = random.random()
            # 0.2 test; 0.08 validation; 0.72 train
            if dice < 0.08:
                samples_validation.append((seq, label))
            elif dice < 0.28:
                samples_test.append((seq, label))
            else:
                samples_training.append((seq, label))
    ...
    def _prepare_dataset(self, seqs):
        ...
        for (seq, label) in seqs:
            total_seqs.append((self._string_to_one_hot(seq), label))
            total_seqs.append((self._string_to_one_hot(self._reverse_complement(seq)), label))
...
def main():
    ...
    dp = datasetPrep()
    labelled_list, n_pos, n_neg = dp.prepare_labelled_samples(options.dir_data, options.TF_name)
    sequences = dp.convert(labelled_list, options.len_seq, options.dir_data)
    training, validation, test = dp.make_datasets(sequences)
