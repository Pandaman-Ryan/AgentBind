...
def main():
    ...
    parser.add_option('--motif', dest='f_core_motif',
            default="/storage/pandaman/project/AgentBind-GM12878-DanQ-unfixed-rnn-trans/storage/AgentBind-GM12878-DanQ/data/table_matrix/table_core_motifs.txt",
    ...
    parser.add_option('--workdir', dest='dir_work',
            default='/storage/pandaman/project/AgentBind-GM12878-DanQ-unfixed-rnn-trans/storage/AgentBind-GM12878-DanQ/tmp/',
    ...
    parser.add_option('--datadir', dest='dir_data',
            default='/storage/pandaman/project/AgentBind-GM12878-DanQ-unfixed-rnn-trans/storage/AgentBind-GM12878-DanQ/data/',
    ...
    parser.add_option('--resultdir', dest='dir_result',
            default='/storage/pandaman/project/AgentBind-GM12878-DanQ-unfixed-rnn-trans/storage/AgentBind-GM12878-DanQ/results/',
            help='The directory where all input and dependent data are stored [Default: %default]')
    ...
    '''
    for core_block_suffix in ['', 'c']: #'b' stand for block, 'c' stand for core
        ...
        res_stat_path = "%s/residue_stats.txt" %(summary_path)
        if os.path.isfile(res_stat_path):
            os.remove(res_stat_path)
    ...
    # comment the following before release
    '''
    ...
    for (TF_name, TF_ID, TF_chipseq) in TF_list:
        ...
        for suffix in ['c']: #'b' stand for block, 'c' stand for core
            ...
            analysis_cmd = "python ./data_analysis/data_analysis.py "\
                    " --datadir %s --storage %s --fweight %s --negcoord %s "\
                    " --fimodir %s --resultdir %s "\
                    " --TFname %s " %(options.dir_data, dir_analysis_tmp_data, f_weight, f_neg_coord,
    ...
    for suffix in ['c']:
        ...
        for line in open(summary_path):
            (TF_name, AUC_value) = line.strip().split()
            if TF_name not in AUC_dict:
                AUC_dict[TF_name] = {}
            AUC_dict[TF_name][suffix] = {'AUC': AUC_value}

        res_stat_path = "%s/%s/residue_stats.txt" %(options.dir_result, suffix)
        for line in open(res_stat_path):
            (TF_name, bds_ratio) = line.strip().split()
            if TF_name not in AUC_dict:
                exit("TF not in AUC_dict: %s" %(TF_name))
            AUC_dict[TF_name][suffix]['bds_ratio'] = bds_ratio
        ...
        for TF_name in AUC_dict:
            if ('b' not in AUC_dict[TF_name])\
                ...
                line_to_save = "%s\t%s\t%s\t%s\t%s\n" %(TF_name,
                        AUC_dict[TF_name]['c']['AUC'], AUC_dict[TF_name]['c']['bds_ratio'],
                        AUC_dict[TF_name]['b']['AUC'], AUC_dict[TF_name]['b']['bds_ratio'])
