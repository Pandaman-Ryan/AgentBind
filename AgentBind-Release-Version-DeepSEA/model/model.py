...
class AgentBind():
    ...
    def infer(self, samples, training, trainable_conv_layers=True):
        ...
        conv1 = self._conv_layer(inputs=samples, scope_name='conv1',
                        filter_width=8, in_channels=4, out_channels=320, conv_stride=1,
                        pool_op=True, pool_window_size=4, pool_stride=4, training=training,
                        trainable_parameters=trainable_conv_layers, dropout=0.2, record='conv')
        conv2 = self._conv_layer(inputs=conv1, scope_name='conv2',
                        filter_width=8, in_channels=320, out_channels=480, conv_stride=1,
                        pool_op=True, pool_window_size=4, pool_stride=4, training=training,
                        trainable_parameters=trainable_conv_layers, dropout=0.2, record=None)
        conv3 = self._conv_layer(inputs=conv2, scope_name='conv3',
                        filter_width=8, in_channels=480, out_channels=960, conv_stride=1,
                        pool_op=False, training=training,
                        trainable_parameters=trainable_conv_layers, dropout=0.5, record=None)
    ...
    def _conv_layer(self, inputs, scope_name,
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            pre_activation_norm = tf.layers.batch_normalization(pre_activation,
                                        training=training, trainable=True)
    ...
    def _fully_connected_layer(self, inputs, scope_name,
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            biases = tf.get_variable(name='biases', shape=[out_neurons],
                                    initializer=tf.constant_initializer(0.0), trainable=trainable_parameters)
            if activated:
                fc = tf.nn.relu(
