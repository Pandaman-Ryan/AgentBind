...
def _print_summary(scores=None, logits=None, info=None):
    weights_total = np.zeros(scores.shape[1:])
    # TODO: now this function can only deal with one class
    for sample_index in range(FLAGS.n_samples):
        feature_index = 0 # because there is only one class in this model
        lgt = logits[sample_index, feature_index]

        #lgt_ref = lgt[0]
        intensity = float(info[sample_index, 0])
        pval = float(info[sample_index, 1])
        position = int(info[sample_index, 2])
        chromID = str(info[sample_index, 3])
        motif_start = int(info[sample_index, 4])
        motif_end = int(info[sample_index, 5])
        seq_start = int(info[sample_index, 6])
        seq_end = int(info[sample_index, 7])
        ...
        for i, w in enumerate(filter_weights):
            cam += w * cam_vals[:, i]
        cam_resized = resize(cam, (len(weights_ref),), preserve_range=True, mode='edge')
        ...
        # figure 1: plot weight distribution
        if sample_index < 100:
            # save weights into a file with their corresbounding positions
            ...
            plt.xlabel('positions-predscore:%.2f-intensity:%.2f-pval:%s\n%s-%d-%d'\
                            %(lgt, intensity, pval, chromID, seq_start, seq_end))
            plt.ylabel('weights')
            plt.savefig('%s/vis-sample-%d.png' %(FLAGS.figure_dir, sample_index))
            plt.close()
        
    # general landscape
    for idx in range(len(weights_total)):
        weights_total[idx] /= float(FLAGS.n_samples)
    # write into txt
    with open ('%s/vis-sample-total.txt' %(FLAGS.figure_dir), 'w') as ofile:
        for pos in range(FLAGS.seq_size):
            line_to_print = "%d\t%s\n" %(pos, weights_total)
            ofile.write(line_to_print)
    # plot
    plt.figure(figsize=(11,11))
    plt.scatter([pos for pos in range(FLAGS.seq_size)], weights_total, marker=".")
    plt.title("signal vs positions")
    plt.xlabel('positions')
    plt.ylabel('weights')
    plt.savefig('%s/vis-sample-total.png' %(FLAGS.figure_dir))
    plt.close()
    return
...
def test():
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            num_iter = int(math.ceil(n_eval_samples / FLAGS.batch_size))
