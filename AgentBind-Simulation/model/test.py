...
def _print_summary(losses, logits=None, labels=None):
    ...
    with open(summary_file,'a') as ofile:
        line_to_save = "%s\t%f\t%f\n" %(FLAGS.TF_name, auc_value, pr_value)
    ...
    with tf.Graph().as_default() as g:
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            while True:
                step += 1
                try:
                    logits_value, labels_value, loss_value = sess.run(
                                                [logits_sigmoid, labels, loss],
            ...
            _print_summary(losses, logits_value_total, labels_value_total)
