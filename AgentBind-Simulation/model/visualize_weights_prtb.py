...
def _print_summary(logits=None, info=None, seqs=None, n_samples_total=None):
    '''
        Evalutions:
    ...
    n_samples = int(n_samples_total/(3*FLAGS.seq_size+1))
    for sample_index in range(n_samples):
        offset = sample_index*(3*FLAGS.seq_size+1)
        position_info = info[offset, 0]
        lgt_ref = logits[offset, 0] #feature_index = 0, because there is only one class in this model
        #
        data_to_vis = []
        for pos in range(FLAGS.seq_size):
            lgt_diff_max = 0
            for nt_index in range(3):
                lgt = logits[offset+1+3*pos+nt_index, 0] #feature_index = 0, because there is only one class in this model
                lgt_diff_current = abs(lgt_ref - lgt)
                if lgt_diff_current >= lgt_diff_max:
                    lgt_diff_max = lgt_diff_current

            lgt_diff_max += 10-9
            data_to_vis.append(lgt_diff_max)
...
def test():
    ...
    with tf.Graph().as_default() as g:
        ...
        test_dataset = agent.load(data_dir=FLAGS.data_dir+"/vis-samples-prtb/",
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            _print_summary(logits_value_total, info_value_total, seqs_value_total, FLAGS.n_samples)
