...
def _print_summary(weights=None, cam_values=None, cam_gradients=None, logits=None, info=None, seqs=None, n_samples=None):
    '''
        Evalutions:
    ...
    for sample_index in range(n_samples):
        ...
        cam_grads = cam_gradients[sample_index] #(100, 50)
        cam_vals = cam_values[sample_index] #(100, 50)


        filter_weights = np.mean(cam_grads, axis=0)
        cam = np.zeros(cam_vals.shape[0], dtype=np.float32) #(100)
        print (np.maximum(filter_weights - 1e-3, 0) +\
                (np.minimum(filter_weights + 1e-3, 0)))
        for i, w in enumerate(filter_weights):
            cam += w * cam_vals[:, i]
        cam = np.maximum(cam, 0)
        cam_resized = resize(cam, len(wgt))
        #cam_resized = resize(cam, (len(wgt),), preserve_range=True, mode='edge')
